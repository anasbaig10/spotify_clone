...
function removeFromPlaylist(button, playlistId) {
	...
	$.post("includes/handlers/ajax/removeFromPlaylist.php", { playlistId: playlistId, songId: songId}).done(function(error) {
			// do soemthing when ajax request
			if(error != "") {
				alert(error);
				return;
			}
...
function deletePlaylist(playlistId) {
	var prompt = confirm("Are you sure you wan to delete this playlist?");
...
function showOptionsMenu(button) {
	...
	var scrollTop = $(window).scrollTop(); //distance fronttop of window to top of document
